...
class VisualFrameFeatureTrackVisualizer {
 ...
 private:
  typedef aslam::AlignedUnorderedMap<int, size_t> TrackIdToIndexMap;

  void preprocessLastFrame(TrackIdToIndexMap* track_id_to_keypoint_index_map);

  struct Track {
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    aslam::Aligned<std::vector, Eigen::Vector2d> keypoints;
    cv::Scalar color;
  };
  typedef aslam::AlignedUnorderedMap<int, Track> TrackMap;
