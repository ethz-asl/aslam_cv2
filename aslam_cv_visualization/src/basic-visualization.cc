...
void drawKeypoints(const aslam::VisualFrame& frame, cv::Mat* image) {
  ...
  for (size_t keypoint_idx = 0u; keypoint_idx < num_keypoints; ++keypoint_idx) {
    ...
    cv::circle(
        *image, cv::Point(keypoint[0], keypoint[1]), 1, cv::Scalar(0, 255, 255),
        1, CV_AA);
  }
}
...
void visualizeKeypoints(
  ...
  for (size_t frame_idx = 0; frame_idx < num_frames; ++frame_idx) {
    ...
    cv::Mat frame_slice_of_multi_image = image(
        cv::Rect(
            offsets[frame_idx].width, offsets[frame_idx].height, image_width,
            image_height));
...
void drawKeypointMatches(
    ...
    const aslam::Matches& matches_kp1_k, cv::Scalar color_keypoint_kp1,
    cv::Scalar line_color, cv::Mat* image) {
  CHECK_NOTNULL(image);
  ...
  for (const aslam::Match& match_kp1_k : matches_kp1_k) {
    ...
    if (frame_k.getRawCameraGeometry()) {
      ...
      frame_kp1.getKeypointInRawImageCoordinates(
          match_kp1_k.first, &keypoint_kp1);
    } else {
    ...
    cv::circle(
        *image, cv::Point(keypoint_kp1[0], keypoint_kp1[1]), 4,
        color_keypoint_kp1);
    cv::line(
        *image, cv::Point(keypoint_k[0], keypoint_k[1]),
        cv::Point(keypoint_kp1[0], keypoint_kp1[1]), line_color);
  }
}

void assembleMultiImage(
    const aslam::VisualNFrame::ConstPtr& nframe, cv::Mat* full_image_ptr,
    Offsets* offsets_ptr) {
  CHECK(nframe);
  const size_t num_frames = nframe->getNumFrames();
  ...
  size_t num_rows = static_cast<size_t>(
      std::floor(std::sqrt(static_cast<double>(num_frames))));
  size_t num_images_per_row = static_cast<size_t>(
      std::ceil(
          static_cast<double>(num_frames) / static_cast<double>(num_rows)));
  ...
  for (size_t frame_idx = 0u; frame_idx < num_frames; ++frame_idx) {
    ...
    cv::cvtColor(
        nframe->getFrame(frame_idx).getRawImage(), individual_images[frame_idx],
        CV_GRAY2BGR);
    ...
    VLOG(4) << "Adding image of dimension " << image_width << " x "
            << image_height;
  ...
  VLOG(3) << "Reshaped full image to the following dimensions: " << max_column
          << " x " << row_index;
  ...
  for (size_t frame_idx = 0u; frame_idx < num_frames; ++frame_idx) {
    ...
    VLOG(5) << "Accessing slice of full image starting at ("
            << offsets[frame_idx].width << " , " << offsets[frame_idx].height
            << ").";
    cv::Mat slice = full_image(
        cv::Rect(
            offsets[frame_idx].width, offsets[frame_idx].height, image_width,
            image_height));
...
void assembleMultiImage(
    const std::vector<cv::Mat>& images, cv::Mat* full_image_ptr,
    Offsets* offsets_ptr) {
  const size_t num_frames = images.size();
  CHECK_GT(num_frames, 0u);
  CHECK_NOTNULL(full_image_ptr);
  CHECK_NOTNULL(offsets_ptr);
  ...
  for (size_t frame_idx = 0u; frame_idx < num_frames; ++frame_idx) {
    ...
    if (images[frame_idx].type() == CV_8UC1) {
      cv::cvtColor(
          images[frame_idx], individual_images[frame_idx], CV_GRAY2BGR);
    } else {
      individual_images[frame_idx] = images[frame_idx];
    }

    const size_t image_width = static_cast<size_t>(images[frame_idx].cols);
    const size_t image_height = static_cast<size_t>(images[frame_idx].rows);
...
void drawFeatureTrackPatches(
    const aslam::FeatureTrack& track, size_t keypoint_neighborhood_px,
  ...
  const size_t num_subimage_rows =
      std::ceil(track_length / static_cast<double>(num_cols)) *
      subimage_edge_length_px;
  ...
  for (const aslam::KeypointIdentifier& kip : track.getKeypointIdentifiers()) {
    ...
    cv::cvtColor(
        keypoint_image(roi_keypoint), keypoint_neighbourhood_image,
        CV_GRAY2BGR);
    cv::Point keypoint_coords_subimage(
        keypoint[0] - roi_keypoint.x, keypoint[1] - roi_keypoint.y);
    cv::circle(
        keypoint_neighbourhood_image, keypoint_coords_subimage, 1,
...
bool drawFeatureTracksPatches(
  ...
  for (const aslam::FeatureTrack& track : tracks) {
    ...
    cv::Rect roi_track(
        cv::Point(0, track_idx * edge_size),
...
bool drawFeatureTracks(const aslam::FeatureTracks& tracks, cv::Mat* image) {
  ...
  const cv::Mat& frame_image =
      tracks.front().getLastKeypointIdentifier().getFrame().getRawImage();
  ...
  for (const aslam::FeatureTrack& track : tracks) {
    ...
    for (const aslam::KeypointIdentifier& kip :
...
void drawLines(const aslam::Lines2d& lines, cv::Mat* image) {
  ...
  for (size_t idx = 0u; idx < lines.size(); ++idx) {
    const cv::Point2d start_point(
        lines[idx].getStartPoint().x(), lines[idx].getStartPoint().y());
    const cv::Point2d end_point(
        lines[idx].getEndPoint().x(), lines[idx].getEndPoint().y());
