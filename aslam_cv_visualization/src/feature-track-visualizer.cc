...
void VisualFrameFeatureTrackVisualizer::drawContinuousFeatureTracks(
  ...
  if (last_frame_) {
    ...
    for (size_t idx = 0; idx < num_track_ids; ++idx) {
      ...
      if (track_id >= 0) {
        ...
        if (active_track_it == track_id_to_track_map_.end()) {
          // This is a new track. Get the first two keypoints using the last
          // frame.
          ...
          CHECK(last_frame_it != last_frame_track_id_to_index_map.end())
              << "Could not find track with ID " << track_id
              << " in the last frame, although this is a new track. "
              << "Current keypoint index: " << idx;
          CHECK_LT(
              last_frame_it->second, last_frame_->getNumKeypointMeasurements());

          Track new_track;
          new_track.color = cv::Scalar(
              rng_.uniform(0, 255), rng_.uniform(0, 255), rng_.uniform(0, 255));
    ...
    for (TrackMap::const_iterator track_it = track_id_to_track_map_.begin();
      ...
      Aligned<std::vector, Eigen::Vector2d>::const_iterator measurement_it =
          track_it->second.keypoints.begin();
      ...
      for (; measurement_it != track_it->second.keypoints.end();
        ...
        if (terminated) {
          cv::line(
              *image, point_start_line, point_end_line, cv::Scalar(0, 0, 255),
              kLineWidth, CV_AA);
        } else {
          cv::line(
              *image, point_start_line, point_end_line, track_it->second.color,
              kLineWidth, CV_AA);
        }
      ...
      if (!terminated) {
        cv::circle(
            *image, point_start_line, kCircleRadius,
            cv::Scalar(0.0, 255.0, 255.0, 20), kLineWidth, CV_AA);
      }
    }
...
void VisualFrameFeatureTrackVisualizer::preprocessLastFrame(
  ...
  CHECK_EQ(last_frame_num_track_ids, last_frame_->getNumKeypointMeasurements());
...
VisualNFrameFeatureTrackVisualizer::VisualNFrameFeatureTrackVisualizer(
...
void VisualNFrameFeatureTrackVisualizer::drawContinuousFeatureTracks(
    ...
    const aslam::FeatureTracksList& terminated_feature_tracks, cv::Mat* image) {
  CHECK(nframe);
  CHECK_NOTNULL(image);
  ...
  for (size_t frame_idx = 0; frame_idx < num_frames; ++frame_idx) {
    ...
    feature_track_visualizers_[frame_idx].drawContinuousFeatureTracks(
        nframe->getFrameShared(frame_idx), terminated_feature_tracks[frame_idx],
        &slice);
  }
}
