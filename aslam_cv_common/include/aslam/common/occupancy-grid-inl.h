#ifndef ASLAM_COMMON_OCCUPANCY_GRID_INL_H_
#define ASLAM_COMMON_OCCUPANCY_GRID_INL_H_
...
WeightedOccupancyGrid<PointType>::WeightedOccupancyGrid(
  ...
  num_grid_rows_ = static_cast<size_t>(
      std::ceil(
          static_cast<double>(max_input_coordinate_rows) / cell_size_rows_));
  num_grid_cols_ = static_cast<size_t>(
      std::ceil(
...
template <typename PointType>
void WeightedOccupancyGrid<PointType>::addPointUnconditional(
    const PointType& point) {
  GridCoordinates grid_coordinates =
      inputToGridCoordinates(point.u_rows, point.v_cols);
...
bool WeightedOccupancyGrid<PointType>::addPointOrReplaceWeakestIfCellFull(
  ...
  if (cell.size() >= max_points_per_cell) {
    typename PointList::iterator it_min_weight =
...
void WeightedOccupancyGrid<PointType>::addPointOrReplaceWeakestNearestPoints(
  ...
  CHECK_LE(
      min_distance,
      static_cast<CoordinatesType>(std::min(cell_size_cols_, cell_size_rows_)))
      << "Distances are only checked in the closest neighboring cells, "
         "therefore the min. distance "
      << "has to be smaller or equal to the cell size.";

  // As the min. distance has to be smaller than the grid size, we only need to
  // check points in the
  // cells that are direct neighbors.
  GridCoordinates cell_input_point =
      inputToGridCoordinates(point_to_insert.u_rows, point_to_insert.v_cols);
  ...
  size_t i_row_max = std::min(
      static_cast<int>(cell_input_point.i_rows) + 1,
      static_cast<int>(num_grid_rows_ - 1));
  size_t j_col_max = std::min(
      static_cast<int>(cell_input_point.j_cols) + 1,
      static_cast<int>(num_grid_cols_ - 1));

  // Collect all points from the neighboring cells that violate the min.
  // distance to the point
  // that should be inserted.
  const CoordinatesType min_distance_sq = min_distance * min_distance;

  typedef std::unordered_map<PointList* /*cell*/,
                             std::vector<size_t> /*point index*/>
      PointIdentifierMap;
  ...
  for (size_t i_row = i_row_min; i_row <= i_row_max; ++i_row) {
    for (size_t j_col = j_col_min; j_col <= j_col_max; ++j_col) {
      ...
      // Add all points of this cell to the violation list if it is too close to
      // the input point.
  ...
  CHECK_LT(
      highest_weight_violator_idx,
      distance_violating_points[highest_weight_cell].size());

  // Exclude the point with the highest weight from the list of points to
  // remove.
  distance_violating_points[highest_weight_cell].erase(
      distance_violating_points[highest_weight_cell].begin() +
      highest_weight_violator_idx);
  ...
  for (const typename PointIdentifierMap::value_type& cell_indices_pair :
       distance_violating_points) {
    PointList& cell = *CHECK_NOTNULL(cell_indices_pair.first);
    const std::vector<size_t>& indices_to_remove_from_cell =
        cell_indices_pair.second;
    PointList reduced_cell = aslam::common::eraseIndicesFromVector(
        cell, indices_to_remove_from_cell);
...
void WeightedOccupancyGrid<PointType>::addPointOrReplaceWeakestNearestPoints(
    const PointType& point_to_insert, CoordinatesType min_distance) {
  CHECK_GT(min_distance, static_cast<CoordinatesType>(0.0));
  CHECK_LE(min_distance, static_cast<CoordinatesType>(std::min(cell_size_cols_,
cell_size_rows_)))
    << "Distances are only checked in the closest neighboring cells, therefore
the min. distance "
    << "has to be smaller or equal to the cell size.";
...
check points in the
  // cells that are direct neighbors.
  GridCoordinates cell_input_point =
inputToGridCoordinates(point_to_insert.u_rows,
...
the input point.
      for (size_t point_idx = 0u; point_idx < cell.size(); ++point_idx) {
        ...
        if (delta.squaredNorm() < min_distance_sq) {
          distance_violating_points[&cell].emplace_back(point_idx);
          //LOG(ERROR) << "Adding point with id: " << cell[point_idx].id << "
and weight "
...
cell_indices_pair.second;
    ...
    for (size_t cell_point_index = 0u; cell_point_index <
indices_to_remove_from_cell.size();
         ++cell_point_index) {
      ...
      if (point_weight >= highest_score) {
        highest_point_id =
cell_to_check[indices_to_remove_from_cell[cell_point_index]].id;
  ...
  // Remove the point with the highest score from the list of points to remove
from the grid.
  CHECK_NOTNULL(cell_highest_score);
  CHECK_LT(index_highest_score,
distance_violating_points[cell_highest_score].size());
  distance_violating_points[cell_highest_score].erase(
      distance_violating_points[cell_highest_score].begin() +
index_highest_score);
  ...
  for (const typename PointIdentifierMap::value_type& cell_indices_pair :
    ...
    const std::vector<size_t>& indices_to_remove_from_cell =
cell_indices_pair.second;
    cell = aslam::common::eraseIndicesFromVector(cell,
indices_to_remove_from_cell);
    current_num_points_ -= indices_to_remove_from_cell.size();
  }
}*/
...
template <typename PointType>
void WeightedOccupancyGrid<PointType>::setConstantWeightForAllPointsInGrid(
    WeightType weight) {
  for (size_t i_row = 0u; i_row < num_grid_rows_; ++i_row) {
    for (size_t j_col = 0u; j_col < num_grid_cols_; ++j_col) {
      // for (size_t j_col = 0u; j_col < num_grid_cols_; ++j_col) {
...
cv::Mat WeightedOccupancyGrid<PointType>::getOccupancyMask(
  ...
  // Go over all cells and mask either the entire cell if the max. point count
  // has been reached
  // in this cell or otherwise mask out the point in this cell.
  cv::Mat mask(
      static_cast<int>(std::floor(max_input_coordinate_rows_)),
      static_cast<int>(std::floor(max_input_coordinate_cols_)), CV_8UC1,
      cv::Scalar(255));
  ...
  for (size_t i_row = 0u; i_row < num_grid_rows_; ++i_row) {
    for (size_t j_col = 0u; j_col < num_grid_cols_; ++j_col) {
      ...
      for (const PointType& point : cell) {
        cv::circle(
            mask, cv::Point(point.v_cols, point.u_rows),
            radius_mask_around_points, cv::Scalar(0), CV_FILLED);
      }
...
WeightedOccupancyGrid<PointType>::inputToGridCoordinates(
    CoordinatesType u_rows, CoordinatesType v_cols) {
  CHECK(isValidInputCoordinate(u_rows, v_cols)) << "u_rows: " << u_rows
                                                << ", v_cols: " << v_cols;
  const size_t grid_row =
      static_cast<size_t>(std::floor(u_rows / cell_size_rows_));
  const size_t grid_col =
      static_cast<size_t>(std::floor(v_cols / cell_size_cols_));
...
#endif  // ASLAM_COMMON_OCCUPANCY_GRID_INL_H_
