#ifndef ASLAM_COMMON_CHANNEL_DECLARATION_H_
#define ASLAM_COMMON_CHANNEL_DECLARATION_H_
#include <memory>
#include <mutex>
#include <string>
...
#define DECLARE_CHANNEL_IMPL(NAME, TYPE)                                       \
  ...
  NAME##_ChannelValueType& get_##NAME##_Data(                                  \
    ...
    return derived->value_;                                                    \
                                                                               ...
                                                                               \
  NAME##_ChannelValueType& add_##NAME##_Channel(ChannelGroup* channel_group) { \
...
CHANNEL_DATA_TYPE& getChannelData(
    const std::string& channel_name, const ChannelGroup& channel_group) {
  std::lock_guard<std::mutex> lock(channel_group.m_channels_);
...
template <typename CHANNEL_DATA_TYPE>
CHANNEL_DATA_TYPE& addChannel(
    const std::string& channel_name, ChannelGroup* channel_group) {
  CHECK_NOTNULL(channel_group);
  std::lock_guard<std::mutex> lock(channel_group->m_channels_);
...
#endif  // ASLAM_COMMON_CHANNEL_DECLARATION_H_
