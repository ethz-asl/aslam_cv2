#ifndef ASLAM_COMMON_OPENCV_PREDICATES_H_
#define ASLAM_COMMON_OPENCV_PREDICATES_H_
...
inline testing::AssertionResult ImagesEqualTyped(
    const cv::Mat& A, const cv::Mat& B, double tolerance) {
  typedef SCALAR Scalar;
  bool success = true;
  if (A.rows != B.rows || A.cols != B.cols || A.channels() != B.channels()) {
    return testing::AssertionFailure()
           << "Image size mismatch: " << A.rows << "x" << A.cols << "x"
           << A.channels() << " != " << B.rows << "x" << B.cols << "x"
           << B.channels();
  }
  ...
  for (int r = 0; r < A.rows; ++r) {
    for (int c = 0; c < A.cols; ++c) {
      for (int ch = 0; ch < A.channels(); ++ch) {
        ...
        if (!std::isfinite(Aij) || !std::isfinite(Bij) ||
            !Eigen::internal::isApprox(Aij, Bij, tolerance)) {
          spy_difference << "x";
          if (A.rows == 1) {
            failure_reason << "\nMismatch at position " << c << ": " << Aij
                           << " != " << Bij;
          } else if (A.cols == 1) {
            failure_reason << "\nMismatch at position " << c << ": " << Aij
                           << " != " << Bij;
...
inline testing::AssertionResult ImagesEqual(
    const cv::Mat& A, const cv::Mat& B, double tolerance) {
  if (A.type() != B.type()) {
    return testing::AssertionFailure() << "Image type mismatch" << A.type()
                                       << " != " << B.type();
  }
  ...
  switch (A.depth()) {
    ...
    case cv::DataType<float>::depth:
      ...
      break;
    default:
      return ::testing::AssertionFailure() << "cv::Mat depth " << A.depth()
...
#endif  // ASLAM_COMMON_OPENCV_PREDICATES_H_
