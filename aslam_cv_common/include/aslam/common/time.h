#ifndef ASLAM_COMMON_TIME_H_
#define ASLAM_COMMON_TIME_H_
...
template <typename TimeUnit>
inline static int64_t convertToNanoseconds(const double value) {
  return static_cast<int64_t>(value * time_traits<TimeUnit>::nanoseconds);
}
template <typename TimeUnitOut, typename TimeUnitIn = double>
inline static double convertFromNanoseconds(const TimeUnitIn value) {
  return static_cast<double>(value) /
         static_cast<double>(time_traits<TimeUnitOut>::nanoseconds);
}
}  // namespace internal
...
/// Example: double milliseconds = aslam::time::to_milliseconds(int64_t
...
/// Example: int64_t timestamp_nanoseconds = aslam::time::from_seconds(double
/// seconds);
...
inline std::string timeNanosecondsToString(
