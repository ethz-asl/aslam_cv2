#ifndef ASLAM_COMMON_CHANNEL_EXTERNAL_DECLARATION_H_
#define ASLAM_COMMON_CHANNEL_EXTERNAL_DECLARATION_H_
...
#define DECLARE_EXTERNAL_CHANNEL_HELPERS_IMPL(ChannelName, ChannelType) \
  inline const ChannelType& get##ChannelName(                           \
      const aslam::VisualFrame& frame) {                                \
    return frame.getChannelData<ChannelType>(k##ChannelName);           \
  }                                                                     \
  inline void set##ChannelName(                                         \
      const ChannelType& data, aslam::VisualFrame* frame) {             \
    CHECK_NOTNULL(frame);                                               \
...
#define DECLARE_EXTERNAL_CHANNEL_SWAP_IMPL(ChannelName, ChannelType) \
  inline void swap##ChannelName(                                     \
      ChannelType* data, aslam::VisualFrame* frame) {                \
    CHECK_NOTNULL(frame);                                            \
...
// Declare an additional channel for an aslam:VisualFrame. The underlying type
// does not provide a swap method.
...
// Declare an additional channel for an aslam:VisualFrame. The underlying type
// provides a method to swap data.
...
// Example:
//  The macros below will implement the following functions in the calling
//  scope.
//   -> const Eigen::VectorXi& getGroundTruthLandmarkIds(const
//   aslam::VisualFrame& frame)
//   -> void setGroundTruthLandmarkIds(const Eigen::VectorXi& data,
//   aslam::VisualFrame* frame)
//   -> void addGroundTruthLandmarkIds(aslam::VisualFrame* frame)
//   -> void swapGroundTruthLandmarkIds(Eigen::VectorXi* data,
//   aslam::VisualFrame* frame)
//
//  namespace simulation {
//  namespace channels {
//    DECLARE_EXTERNAL_CHANNEL_WITH_SWAP(GroundTruthLandmarkIds,
//    Eigen::VectorXi)
//  }
//  }

#endif  // ASLAM_COMMON_CHANNEL_EXTERNAL_DECLARATION_H_
