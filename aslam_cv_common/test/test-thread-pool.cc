...
TEST(ThreadPoolTests, testOrdered) {
  ...
  constexpr size_t kGroupId = 0u;
  auto job1 = pool.enqueueOrdered(&increment, kGroupId, 500);
  auto job2 = pool.enqueueOrdered(&increment, kGroupId, 400);
  auto job3 = pool.enqueueOrdered(&increment, kGroupId, 300);
  auto job4 = pool.enqueueOrdered(&increment, kGroupId, 200);
  auto job5 = pool.enqueueOrdered(&increment, kGroupId, 100);
  pool.waitForEmptyQueue();
...
TEST(ThreadPoolTests, ExecutionOrdering) {
  ...
  size_t kNumNumbers = 1e3;
  for (int number = 0; number < kNumNumbers; ++number) {
    for (int group_id = 0; group_id < kNumGroups; ++group_id) {
      pool.enqueueOrdered(task, group_id, group_id, number);
    }
  }
  pool.waitForEmptyQueue();
  ...
  double value;
  for (int group_id = 0; group_id < kNumGroups; ++group_id) {
    ASSERT_EQ(receive_queues.at(group_id).size(), kNumNumbers);
    for (int number = 0; number < kNumNumbers; ++number) {
      EXPECT_EQ(receive_queues.at(group_id).at(number), number);
    }
  }
}
