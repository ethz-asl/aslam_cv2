...
TEST(StlHelpers, EraseIndices) {
  ...
  ASSERT_EQ(
      result_vector.size(), test_vector.size() - indices_to_remove.size());
...
TEST(StlHelpers, DrawRandom) {
  ...
  EXPECT_DEATH(
      aslam::common::drawNRandomElements(kNumToPick, test_vector, &test_vector),
      "");
...
TEST(StdHelpers, CountNestedListElements) {
  ...
  size_t num_elements =
      aslam::common::countNumberOfElementsInNestedList(eigen_nested_list);
  ...
  num_elements =
      aslam::common::countNumberOfElementsInNestedList(std_nested_list);
