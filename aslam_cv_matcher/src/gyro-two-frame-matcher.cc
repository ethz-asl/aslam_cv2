...
DEFINE_int32(
    gyro_matcher_small_search_distance_px, 10,
    "Small search rectangle size for keypoint matches.");
DEFINE_int32(
    gyro_matcher_large_search_distance_px, 20,
...
GyroTwoFrameMatcher::GyroTwoFrameMatcher(
    ...
    : frame_kp1_(frame_kp1),
      frame_k_(frame_k),
      q_Ckp1_Ck_(q_Ckp1_Ck),
  ...
  CHECK_EQ(
      prediction_success_.size(), predicted_keypoint_positions_kp1_.cols());
...
void GyroTwoFrameMatcher::initialize() {
  ...
  const Eigen::Matrix<unsigned char, Eigen::Dynamic, Eigen::Dynamic>&
      descriptors_kp1 = frame_kp1_.getDescriptors();
  const Eigen::Matrix<unsigned char, Eigen::Dynamic, Eigen::Dynamic>&
      descriptors_k = frame_k_.getDescriptors();

  for (int descriptor_kp1_idx = 0; descriptor_kp1_idx < kNumPointsKp1;
       ++descriptor_kp1_idx) {
    descriptors_kp1_wrapped_.emplace_back(
        &(descriptors_kp1.coeffRef(0, descriptor_kp1_idx)),
  ...
  for (int i = 0; i < kNumPointsKp1; ++i) {
    keypoints_kp1_sorted_by_y_.emplace_back(
        frame_kp1_.getKeypointMeasurement(i), i);
  }

  std::sort(
      keypoints_kp1_sorted_by_y_.begin(), keypoints_kp1_sorted_by_y_.end(),
...
void GyroTwoFrameMatcher::match() {
  ...
  std::vector<bool> is_inferior_keypoint_kp1_matched(is_keypoint_kp1_matched_);
...
void GyroTwoFrameMatcher::matchKeypoint(const int idx_k) {
  ...
  std::fill(
      iteration_processed_keypoints_kp1_.begin(),
      iteration_processed_keypoints_kp1_.end(), false);

  bool found = false;
  bool passed_ratio_test = false;
  int n_processed_corners = 0;
  KeyPointIterator it_best;
  static const unsigned int kDescriptorSizeBits = 8 * kDescriptorSizeBytes;
  ...
  getKeypointIteratorsInWindow(
      predicted_keypoint_position_kp1, small_search_distance_px_,
      &nearest_corners_begin, &nearest_corners_end);
  ...
  for (KeyPointIterator it = nearest_corners_begin; it != nearest_corners_end;
       ++it) {
  ...
  if (!found) {
    ...
    getKeypointIteratorsInWindow(
        predicted_keypoint_position_kp1, large_search_distance_px_,
        &near_corners_begin, &near_corners_end);

    for (KeyPointIterator it = near_corners_begin; it != near_corners_end;
         ++it) {
  ...
  if (passed_ratio_test) {
    CHECK(
        idx_k_to_attempted_match_data_map_
            .insert(std::make_pair(idx_k, current_match_data))
            .second);
    ...
    if (is_keypoint_kp1_matched_[best_match_keypoint_idx_kp1]) {
      if (matching_score >
          kp1_idx_to_matches_iterator_map_[best_match_keypoint_idx_kp1]
              ->getScore()) {
        ...
        kp1_idx_to_matches_iterator_map_[best_match_keypoint_idx_kp1]->setScore(
            matching_score);
        kp1_idx_to_matches_iterator_map_[best_match_keypoint_idx_kp1]
            ->setIndexApple(best_match_keypoint_idx_kp1);
        kp1_idx_to_matches_iterator_map_[best_match_keypoint_idx_kp1]
            ->setIndexBanana(idx_k);
      } else {
      ...
      CHECK(
          kp1_idx_to_matches_iterator_map_
              .emplace(best_match_keypoint_idx_kp1, matches_kp1_k_->end() - 1)
              .second);
    }

    statistics::StatsCollector stats_distance_match(
        "GyroTracker: number of matching bits");
    stats_distance_match.addSample(static_cast<double>(best_score));
  }
  statistics::StatsCollector stats_count_processed(
      "GyroTracker: number of computed distances per keypoint");
  stats_count_processed.addSample(static_cast<double>(n_processed_corners));
}
...
bool GyroTwoFrameMatcher::matchInferiorMatches(
  ...
  CHECK_EQ(
      is_inferior_keypoint_kp1_matched->size(),
      is_keypoint_kp1_matched_.size());
  ...
  for (const int inferior_keypoint_idx_k : inferior_match_keypoint_idx_k_) {
    ...
    for (size_t i = 0u; i < match_data.keypoint_match_candidates_kp1.size();
         ++i) {
      const KeyPointIterator& keypoint_kp1 =
          match_data.keypoint_match_candidates_kp1[i];
      const double matching_score =
    ...
    if (found) {
      ...
      if ((*is_inferior_keypoint_kp1_matched)[best_match_keypoint_idx_kp1]) {
        if (best_matching_score >
          ...
          // the current keypoint of frame (k+1). Hence, the revoked match is
          // the previous match associated with the current keypoint of frame
          // (k+1).
          ...
          erase_inferior_match_keypoint_idx_k.erase(
              revoked_inferior_keypoint_idx_k);
          ...
          kp1_idx_to_matches_iterator_map_[best_match_keypoint_idx_kp1]
              ->setIndexApple(best_match_keypoint_idx_kp1);
          kp1_idx_to_matches_iterator_map_[best_match_keypoint_idx_kp1]
              ->setIndexBanana(inferior_keypoint_idx_k);
        }
      } else {
  ...
  if (erase_inferior_match_keypoint_idx_k.size() > 0u) {
    ...
    std::vector<int>::iterator iter_erase_from = std::remove_if(
        inferior_match_keypoint_idx_k_.begin(),
        inferior_match_keypoint_idx_k_.end(),
