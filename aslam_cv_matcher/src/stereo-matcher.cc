...
void StereoMatcher::match(
  ...
  CHECK_NOTNULL(matches_frame0_frame1)->clear();
  ...
  CHECK_EQ(iteration_processed_keypoints_frame1.size(), kNumPointsFrame1);
  CHECK_EQ(is_keypoint_frame1_matched.size(), kNumPointsFrame1);
  ...
  Aligned<std::vector, KeypointData> keypoints_frame1_sorted_by_y;
  keypoints_frame1_sorted_by_y.reserve(kNumPointsframe1);
  matches_frame0_frame1->reserve(std::min(kNumPointsFrame0, kNumPointsFrame1));

  // corner_row_LUT[i] is the number of keypoints that has y position
  // lower than i in the image.
  std::vector<int> corner_row_LUT;
  corner_row_LUT.reserve(kImageHeight);
  ...
  for (int descriptor_frame1_idx = 0; descriptor_frame1_idx < kNumPointsFrame1;
    ...
    descriptors_frame1_wrapped.emplace_back(
  ...
  for (int i = 0; i < kNumPointsFrame1; ++i) {
    keypoints_frame1_sorted_by_y.emplace_back(
        frame1.getKeypointMeasurement(i), i);
  }

  std::sort(
      keypoints_frame1_sorted_by_y.begin(), keypoints_frame1_sorted_by_y.end(),
  ...
  for (size_t y = 0u; y < kImageHeight; ++y) {
    while (v < kNumPointsFrame1 &&
           y > static_cast<size_t>(
                   keypoints_frame1_sorted_by_y[v].measurement(1))) {
      ++v;
    }
    corner_row_LUT.push_back(v);
  }
  CHECK_EQ(static_cast<int>(corner_row_LUT.size()), kImageHeight);

  // Remember matched keypoints of frame1.
  std::vector<bool> is_keypoint_frame1_matched(kNumPointsFrame1, false);
  for (int i = 0; i < kNumPointsFrame0; ++i) {
    matchKeypoint(
        i, frame0, frame1, kNumPointsFrame1, conrner_row_LUT, is_keypoint_frame1_matched,
        matches_frame0_frame1);
  }
...
void StereoMatcher::matchKeypoint(
    const int idx_k, const aslam::VisualFrame& frame0,
    const aslam::VisualFrame& frame1, const int kNumPointsFrame1,
    std::vector<int>& corner_row_LUT, std::vector<bool>& is_keypoint_frame1_matched,
    StereoMatchesWithScore* matches_frame0_frame1) {
  ...
  // iteration!
  std::vector<bool> iteration_processed_keypoints_frame1(
      kNumPointsFrame1, false);

  std::fill(
      iteration_processed_keypoints_frame1.begin(),
      iteration_processed_keypoints_frame1.end(), false);
  ...
  getKeypointIteratorsInWindow(
      predicted_keypoint_position_frame1, small_search_distance_px_,
  ...
  if (!found) {
    ...
    getKeypointIteratorsInWindow(
        predicted_keypoint_position_frame1, large_search_distance_px_,
...
bool StereoMatcher::matchInferiorMatches(
  ...
  for (const int inferior_keypoint_idx_k :
    ...
    if (found) {
      ...
      if ((*is_inferior_keypoint_frame1_matched)
        ...
        (*is_inferior_keypoint_frame1_matched)[best_match_keypoint_idx_frame1] =
            true;
        matches_frame1_frame0_->emplace_back(
            best_match_keypoint_idx_frame1, inferior_keypoint_idx_k,
            best_matching_score);
        erase_inferior_match_keypoint_idx_k.insert(inferior_keypoint_idx_k);
