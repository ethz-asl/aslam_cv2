...
void pickNRandomRigMatches(
  ...
  for (size_t cam_idx = 0u; cam_idx < num_cameras; ++cam_idx) {
    ...
    if (camera_matches.size() <= n_per_camera) {
      ...
      common::drawNRandomElements(
          n_per_camera, camera_matches, &(*selected_rig_matches)[cam_idx]);
    }
  }
...
size_t extractMatchesFromTrackIdChannel(
  ...
  CHECK_EQ(
      frame_kp1.getRawCameraGeometry().get(),
  ...
  for (int keypoint_idx_kp1 = 0; keypoint_idx_kp1 < track_ids_kp1.rows();
    ...
    if (track_id_kp1 < 0)
      continue;
    track_id_kp1_keypoint_idx_kp1_map.insert(
        std::make_pair(track_id_kp1, keypoint_idx_kp1));
  }
  CHECK_LE(
      track_id_kp1_keypoint_idx_kp1_map.size(),
      frame_kp1.getNumKeypointMeasurements());
  ...
  for (int keypoint_idx_k = 0; keypoint_idx_k < track_ids_k.rows();
    ...
    TrackIdKeypointIdxMap::const_iterator it =
        track_id_kp1_keypoint_idx_kp1_map.find(track_id_k);
...
size_t extractMatchesFromTrackIdChannels(
  ...
  CHECK_EQ(
      nframe_kp1.getNCameraShared().get(), nframe_k.getNCameraShared().get());
  ...
  for (size_t cam_idx = 0; cam_idx < nframe_kp1.getNumCameras(); ++cam_idx) {
    num_matches += extractMatchesFromTrackIdChannel(
        nframe_kp1.getFrame(cam_idx), nframe_k.getFrame(cam_idx),
...
double getUnrotatedMatchPixelDisparityMedian(
  ...
  if (std::fabs(q_kp1_k.w()) == 1.0) {
    // Case with no rotation specified, directly calculate the disparity from
    // the image plane measurements.
    for (size_t cam_idx = 0u; cam_idx < num_cameras; ++cam_idx) {
      const Eigen::Matrix2Xd& keypoints_kp1 =
          nframe_kp1.getFrame(cam_idx).getKeypointMeasurements();
      const Eigen::Matrix2Xd& keypoints_k =
          nframe_k.getFrame(cam_idx).getKeypointMeasurements();
      for (const FrameToFrameMatch& match_kp1_kp : matches_kp1_k[cam_idx]) {
        ...
        disparity_px.emplace_back(
            (keypoints_kp1.col(match_kp1_kp.first) -
             keypoints_k.col(match_kp1_kp.second))
                .norm());
      }
    }
  } else {
    ...
    for (size_t cam_idx = 0u; cam_idx < num_cameras; ++cam_idx) {
      ...
      for (const FrameToFrameMatch& match_kp1_kp : matches_kp1_k[cam_idx]) {
        CHECK_LT(
            static_cast<int>(match_kp1_kp.second),
      ...
      CHECK_EQ(
          static_cast<int>(matches_kp1_k[cam_idx].size()),
          bearing_vectors_k.cols());
      ...
      Eigen::Matrix3Xd bearing_vectors_k_kp1 =
          q_kp1_k.rotateVectorized(bearing_vectors_k);

      // Project the bearing vectors to the frame kp1 and calculate the
      // disparity.
      const Eigen::Matrix2Xd& keypoints_kp1 =
          nframe_kp1.getFrame(cam_idx).getKeypointMeasurements();
      for (int i = 0; i < bearing_vectors_k_kp1.cols(); ++i) {
        if (success[i]) {
          ...
          aslam::ProjectionResult projection_result =
              nframe_kp1.getCamera(cam_idx).project3(
                  bearing_vectors_k_kp1.col(i), &rotated_k_keypoint);
          if (projection_result == aslam::ProjectionResult::KEYPOINT_VISIBLE) {
            ...
            disparity_px.emplace_back(
                (keypoints_kp1.col(kp1_match_index) - rotated_k_keypoint)
                    .norm());
          } else {
...
void getBearingVectorsFromMatches(
  ...
  aslam::common::convertEigenToStlVector(
      frame_kp1.getNormalizedBearingVectors(keypoint_indices_kp1, &success),
      bearing_vectors_kp1);
  aslam::common::convertEigenToStlVector(
      frame_k.getNormalizedBearingVectors(keypoint_indices_k, &success),
      bearing_vectors_k);
}
...
void predictKeypointsByRotation(
  ...
  Eigen::Matrix3Xd bearing_vectors_k;
  camera.backProject3Vectorized(
      frame_k.getKeypointMeasurements(), &bearing_vectors_k,
      prediction_success);
  CHECK_EQ(
      static_cast<int>(prediction_success->size()), bearing_vectors_k.cols());
  CHECK_EQ(
      static_cast<int>(frame_k.getNumKeypointMeasurements()),
      bearing_vectors_k.cols());

  // Rotate the bearing vectors into the frame_kp1 coordinates.
  const Eigen::Matrix3Xd bearing_vectors_kp1 =
      q_Ckp1_Ck.rotateVectorized(bearing_vectors_k);

  // Project the bearing vectors to the frame_kp1.
  std::vector<ProjectionResult> projection_results;
  camera.project3Vectorized(
      bearing_vectors_kp1, predicted_keypoints_kp1, &projection_results);
  CHECK_EQ(predicted_keypoints_kp1->cols(), bearing_vectors_k.cols());
  CHECK_EQ(
      static_cast<int>(projection_results.size()), bearing_vectors_k.cols());

  // Set the success based on the backprojection and projection results and
  // output the initial unrotated keypoint for failed predictions.
