#ifndef ASLAM_MATCHER_MATCHING_ENGINE_EXCLUSIVE_H_
#define ASLAM_MATCHER_MATCHING_ENGINE_EXCLUSIVE_H_
#include <functional>
#include <vector>
...
/// \brief Matches apples with bananas, such that the resulting matches are
/// exclusive, i.e.
///        every banana matches to at most one apple and vice versa. Not every
///        banana might find a matching apple and vice versa. The assignment
///        procedure is greedy and recursive. Iterating over all bananas,
///        starting at banana 0, the best apple is assigned, replacing a
///        previous assignment to this apple iff the current match score is
///        higher. The banana from the replaced matched is then recursively
///        reassigned to the next best apple (if there is any).
...
class MatchingEngineExclusive : public MatchingEngine<MatchingProblem> {
 ...
 private:
  ...
  inline void assignBest(int index_banana) {
    ...
    // Iterate through the next best apple candidates to find the next best fit
    // (if any).
    for (; iterator_to_next_best_apple_[index_banana] !=
           candidates_[index_banana].end();
  ...
  /// \brief List of sorted candidates for a given banana. (i.e.
  /// candidates_[banana_index] refers
  ///        to a list of apples sorted wrt. the matching score (note: it's
  ///        sorted bottom-up, so .begin() points to the worst apple match for
  ///        this banana. use .rbegin() instead to get best candidate.)
  typename MatchingProblem::CandidatesList candidates_;

  /// \brief The temporary matches assigned to each apple. (i.e.
  /// temporary_matches_[apple_index]
  ///        returns the current match for this apple. May change during the
  ...
  ///        (i.e. iterator_to_next_best_apple_[banana_index] points to the next
  ///        best candidate for the given banana. Points to
  ///        candiates_[banana_index].end() if no next best candidate available.
...
bool MatchingEngineExclusive<MatchingProblem>::match(
    MatchingProblem* problem,
    typename MatchingProblem::MatchesWithScore* matches_A_B) {
  timing::Timer method_timer("aslam::MatchingEngineExclusive::match");
  ...
  if (problem->doSetup()) {
    ...
    for (size_t index_banana = 0; index_banana < num_bananas; ++index_banana) {
      ...
      std::sort(
          candidates_[index_banana].begin(), candidates_[index_banana].end(),
          std::greater<typename MatchingProblem::Candidate>());

      iterator_to_next_best_apple_[index_banana] =
          candidates_[index_banana].begin();
    }
    ...
    for (const typename MatchingProblem::Candidate& candidate :
         temporary_matches_) {
      if (candidate.index_apple >= 0) {
        ...
        matches_A_B->emplace_back(
            candidate.index_apple, candidate.index_banana, candidate.score);
      }
    }
...
#endif  // ASLAM_MATCHER_MATCHING_ENGINE_EXCLUSIVE_H_
