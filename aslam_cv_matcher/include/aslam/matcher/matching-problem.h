#ifndef ASLAM_MATCHER_MATCHING_PROBLEM_H_
#define ASLAM_MATCHER_MATCHING_PROBLEM_H_
...
///
/// IMPORTANT NOTE: If you create a new matching problem inheriting from this
/// class, make sure not to forget to add the
/// ASLAM_ADD_MATCH_TYPEDEFS_WITH_ALIASES(
///    NewMatchingProblemClassName, alias_for_getAppleIndex,
///    alias_for_getBananaIndex)
/// macro in the public class body! Otherwise, no matches type will be available
/// for the new problem and compilation will fail.
///
...
class MatchingProblem {
 public:
  struct Candidate {
    ...
    int index_banana;
    double score;
    /// \brief The priority field allows categorizing candidates. Certain
    /// matching engines might
    ...
    Candidate(int _index_apple, int _index_banana, double _score, int _priority)
        : index_apple(_index_apple),
          index_banana(_index_banana),
          score(_score),
          priority(_priority) {}
    ...
    bool operator==(const Candidate& other) const {
      return (this->index_apple == other.index_apple) &&
             (this->score == other.score) && (this->priority == other.priority);
    }
  };
  ...
  ///
  /// \param[out] candidates_for_bananas Candidates from the Apples-list that
  /// could potentially
  ...
  /// Gets called at the beginning of the matching problem; i.e. to setup
  /// kd-trees, lookup tables or the like.
  virtual bool doSetup() = 0;

  /// List of tested match pairs for every banana. This is only retrieved and
  /// stored if the flag 'matcher_store_all_tested_pairs' is set to true.
  CandidatesList all_tested_pairs_;
};
}  // namespace aslam
#endif  // ASLAM_MATCHER_MATCHING_PROBLEM_H_
