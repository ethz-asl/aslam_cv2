#ifndef KAZE_NLDIFFUSION_FUNCTIONS_H_
#define KAZE_NLDIFFUSION_FUNCTIONS_H_
...
void gaussian_2D_convolution(
    const cv::Mat& src, cv::Mat& dst, size_t ksize_x, size_t ksize_y,  // NOLINT
    float sigma);
...
void image_derivatives_scharr(
    const cv::Mat& src, cv::Mat& dst, const size_t xorder,  // NOLINT
    const size_t yorder);
...
void pm_g1(
    const cv::Mat& Lx, const cv::Mat& Ly, cv::Mat& dst,  // NOLINT
    const float k);
...
#endif  // KAZE_NLDIFFUSION_FUNCTIONS_H_
