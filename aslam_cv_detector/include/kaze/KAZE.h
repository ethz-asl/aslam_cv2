...
#include "fed.h"                    // NOLINT
#include "nldiffusion_functions.h"  // NOLINT
...
class KAZE {
 private:
  ...
  std::vector<TEvolution>
      evolution_;  /// Vector for nonlinear diffusion evolution
 ...
 public:
  ...
  KAZE(KAZEOptions& options);  // NOLINT
  ...
  /// This method selects interesting keypoints with local-maximum response in
  /// the nonlinear scale space
  /// @param kpts Vector of keypoints
  void Feature_Detection(std::vector<cv::KeyPoint>& kpts);  // NOLINT

  /// This method computes the descriptors in the nonlinear scale space
  /// @param kpts Vector of keypoints
  /// @param desc Matrix with the feature descriptors
  void Compute_Descriptors(
      std::vector<cv::KeyPoint>& kpts, cv::Mat& desc);  // NOLINT

  /// This method saves the nonlinear scale space into jpg images for
  /// visualization or debugging purposes
  void Save_Scale_Space();

  /// This method saves the feature detector responses of the nonlinear scale
  /// space for visualization or debugging purposes
 ...
 private:
  ...
  /// This method performs the detection of keypoints by using the normalized
  /// score of the Hessian determinant
  /// @param kpts Vector of keypoints
  /// @note We compute features for each of the nonlinear scale space level in a
  /// different processing thread
  void Determinant_Hessian_Parallel(std::vector<cv::KeyPoint>& kpts);  // NOLINT

  /// This method is called by the thread which is responsible of finding
  /// extrema at a given nonlinear scale level
  ...
  void Do_Subpixel_Refinement(std::vector<cv::KeyPoint>& kpts);  // NOLINT
  ...
  void AOS_Step_Scalar(
      cv::Mat& Ld, const cv::Mat& Ldprev, const cv::Mat& c,  // NOLINT
  ...
  /// This method does the Thomas algorithm for solving a tridiagonal linear
  /// system
  /// @note The matrix A must be strictly diagonally dominant for a stable
  /// solution
  void Thomas(
      const cv::Mat& a, const cv::Mat& b, const cv::Mat& Ld,
      cv::Mat& x);  // NOLINT
  ...
  /// 2006
  void Compute_Main_Orientation(cv::KeyPoint& kpt);  // NOLINT

  /// These methods compute the different descriptors (upright, rotation
  /// invariant, extended) of the provided keypoint using first order
  /// derivatives from the nonlinear scale space using an inspired-SURF
  /// descriptor
  ///  @param kpt Input keypoint
  ///  @param desc Descriptor vector
  ///  @note Rectangular grid of 20 s x 20 s. Descriptor Length 64. No
  ///  additional Gaussian weighting is performed. The descriptor is inspired
  ///  from Bay et al., Speeded Up Robust Features, ECCV, 2006
  ...
  /// These methods compute the different descriptors (upright, rotation
  /// invariant, extended) of the provided keypoint using first order
  /// derivatives from the nonlinear scale space using an inspired-MSURF
  /// descriptor
  /// @param kpt Input keypoint
  /// @param desc Descriptor vector
  /// @note Rectangular grid of 24 s x 24 s. Descriptor Length 64. The
  /// descriptor is inspired from Agrawal et al., CenSurE: Center Surround
  /// Extremas for Realtime Feature Detection and Matching, ECCV 2008
  ...
  /// These methods compute the different descriptors (upright, rotation
  /// invariant, extended) of the provided keypoint using first order
  /// derivatives from the nonlinear scale space using an inspired-GSURF
  /// descriptor
  /// @param kpt Input keypoint
  /// @param desc Descriptor vector
  /// @note Rectangular grid of 20 s x 20 s. Descriptor Length 64. No additional
  /// G-SURF descriptor as described in Pablo F. Alcantarilla, Luis M. Bergasa
  /// and Andrew J. Davison, Gauge-SURF Descriptors, Image and Vision Computing
  /// 31(1), 2013
...
void checkDescriptorLimits(
    int& x, int& y, const int width, const int height);  // NOLINT
