...
/* ************************************************************************* */
void gaussian_2D_convolution(
    const cv::Mat& src, cv::Mat& dst, size_t ksize_x, size_t ksize_y,
    float sigma) {
  ...
  cv::GaussianBlur(
      src, dst, cv::Size(ksize_x, ksize_y), sigma, sigma, cv::BORDER_REPLICATE);
}

/* ************************************************************************* */
void image_derivatives_scharr(
    const cv::Mat& src, cv::Mat& dst, const size_t xorder,
    const size_t yorder) {
...
void pm_g2(const cv::Mat& Lx, const cv::Mat& Ly, cv::Mat& dst, const float k) {
  ...
  for (int y = 0; y < sz.height; y++) {
    ...
    for (int x = 0; x < sz.width; x++)
      dst_row[x] =
          1.0 / (1.0 + inv_k * (Lx_row[x] * Lx_row[x] + Ly_row[x] * Ly_row[x]));
  }
}

/* ************************************************************************* */
void weickert_diffusivity(
    const cv::Mat& Lx, const cv::Mat& Ly, cv::Mat& dst, const float k) {
  cv::Size sz = Lx.size();
  float inv_k = 1.0 / (k * k);
  for (int y = 0; y < sz.height; y++) {
    const float* Lx_row = Lx.ptr<float>(y);
...
void charbonnier_diffusivity(
  ...
  for (int y = 0; y < sz.height; y++) {
    ...
    for (int x = 0; x < sz.width; x++) {
      float den =
          sqrt(1.0 + inv_k * (Lx_row[x] * Lx_row[x] + Ly_row[x] * Ly_row[x]));
      dst_row[x] = 1.0 / den;
    }
  }
}
...
/* ************************************************************************* */
float compute_k_percentile(
    const cv::Mat& img, float perc, float gscale, size_t nbins, size_t ksize_x,
    size_t ksize_y) {
...
/* ************************************************************************* */
void compute_scharr_derivatives(
    const cv::Mat& src, cv::Mat& dst, const size_t xorder, const size_t yorder,
    const size_t scale) {
  cv::Mat kx, ky;
...
/* ************************************************************************* */
void compute_derivative_kernels(
    cv::OutputArray kx_, cv::OutputArray ky_, const size_t dx, const size_t dy,
    const size_t scale) {
...
/* ************************************************************************* */
void nld_step_scalar(
    cv::Mat& Ld, const cv::Mat& c, cv::Mat& Lstep, const float stepsize) {
  Lstep = cv::Scalar(0);
...
#pragma omp parallel for schedule(dynamic)
  ...
  for (int i = 1; i < Lstep.rows - 1; i++) {
    ...
    float xneg = (c_row[Lstep.cols - 2] + c_row[Lstep.cols - 1]) *
                 (Ld_row[Lstep.cols - 1] - Ld_row[Lstep.cols - 2]);
    ypos = (c_row[Lstep.cols - 1] + c_row_p[Lstep.cols - 1]) *
           (Ld_row_p[Lstep.cols - 1] - Ld_row[Lstep.cols - 1]);
    yneg = (c_row_m[Lstep.cols - 1] + c_row[Lstep.cols - 1]) *
           (Ld_row[Lstep.cols - 1] - Ld_row_m[Lstep.cols - 1]);
...
/* ************************************************************************* */
bool check_maximum_neighbourhood(
    const cv::Mat& img, int dsize, float value, int row, int col,
    bool same_img) {
  bool response = true;
