...
/* ************************************************************************* */
int save_keypoints(
    std::string& keypointsFile, const std::vector<cv::KeyPoint>& kpts,
...
void compute_inliers_ransac(
    const std::vector<cv::Point2f>& matches, std::vector<cv::Point2f>& inliers,
  ...
  if (use_fund == true)
    H = cv::findFundamentalMat(
        points1, points2, cv::FM_RANSAC, error, 0.99, status);
  else
...
/* ************************************************************************* */
void draw_inliers(
    const cv::Mat& img1, const cv::Mat& img2, cv::Mat& img_com,
  ...
  cv::resize(
      img2, img_aux, cv::Size(img1.cols, img1.rows), 0, 0, cv::INTER_LINEAR);

  for (int i = 0; i < img_com.rows; i++) {
    for (int j = 0; j < img_com.cols; j++) {
      if (j < img1.cols) {
        *(img_com.ptr<unsigned char>(i) + 3 * j) =
            *(img1.ptr<unsigned char>(i) + 3 * j);
        *(img_com.ptr<unsigned char>(i) + 3 * j + 1) =
            *(img1.ptr<unsigned char>(i) + 3 * j + 1);
        *(img_com.ptr<unsigned char>(i) + 3 * j + 2) =
            *(img1.ptr<unsigned char>(i) + 3 * j + 2);
      } else {
        *(img_com.ptr<unsigned char>(i) + 3 * j) =
            *(img2.ptr<unsigned char>(i) + 3 * (j - img_aux.cols));
        *(img_com.ptr<unsigned char>(i) + 3 * j + 1) =
  ...
  for (size_t i = 0; i < ptpairs.size(); i += 2) {
    ...
    cv::line(
        img_com, cv::Point(x1, y1), cv::Point(x2, y2), cv::Scalar(255, 0, 0),
        2);
  }
}
...
void draw_inliers(
  ...
  for (size_t i = 0; i < ptpairs.size(); i += 2) {
    ...
    if (color == 0)
      cv::line(
          img_com, cv::Point(x1, y1), cv::Point(x2, y2),
      ...
      cv::line(
          img_com, cv::Point(x1, y1), cv::Point(x2, y2), cv::Scalar(0, 255, 0),
          2);
  }
}
...
void show_input_options_help(int example) {
  ...
  cout << "***********************************************************" << endl;
  cout << "For running the program you need to type in the command line the "
          "following arguments: "
       << endl;
  ...
  cout << endl;
  cout << "The options are not mandatory. In case you do not specify "
          "additional options, default arguments will be used"
       << endl
       << endl;
  cout << "Here is a description of the additional options: " << endl;
  cout << "--verbose "
       << "\t\t if verbosity is required" << endl;
  cout << "--help"
       << "\t\t for showing the command line options" << endl;
  cout << "--soffset"
       << "\t\t the base scale offset (sigma units)" << endl;
  cout << "--omax"
       << "\t\t maximum octave evolution of the image 2^sigma (coarsest scale)"
       << endl;
  cout << "--nsublevels"
       << "\t\t number of sublevels per octave" << endl;
  cout << "--dthreshold"
       << "\t\t Feature detector threshold response for accepting points "
          "(0.001 can be a good value)"
       << endl;
  cout << "--descriptor"
       << "\t\t Descriptor Type 0 -> SURF, 1 -> M-SURF, 2 -> G-SURF" << endl;
  cout
      << "--use_fed"
         "\t\t 1 -> Use FED, 0 -> Use AOS for the nonlinear diffusion filtering"
      << endl;
  cout << "--upright"
       << "\t\t 0 -> Rotation Invariant, 1 -> No Rotation Invariant" << endl;
  cout << "--extended"
       << "\t\t 0 -> Normal Descriptor (64), 1 -> Extended Descriptor (128)"
       << endl;
  cout << "--output keypoints.txt"
       << "\t\t For saving the detected keypoints into a .txt file" << endl;
  cout << "--save_scale_space"
       << "\t\t 1 in case we want to save the nonlinear scale space images. 0 "
          "otherwise"
       << endl;
  cout << "--show_results"
...
/* ************************************************************************* */
void display_text(
    cv::Mat& img_rgb, int npoints1, int npoints2, int nmatches, int ninliers,
    float ratio, float dratio, int index) {
  char text[400];
  ...
  if (index == 0)
    putText(
        img_rgb, text, cv::Point(20, 30), cv::FONT_HERSHEY_DUPLEX, .75,
        cv::Scalar(255, 255, 0), 2, 8, false);
  else if (index == 1)
    putText(
        img_rgb, text, cv::Point(20, 30), cv::FONT_HERSHEY_DUPLEX, .75,
  ...
  sprintf(
      text, "# Points Image 1: %d, # Points Image 2: %d", npoints1, npoints2);

  if (index == 0)
    putText(
        img_rgb, text, cv::Point(20, img_rgb.rows - 70),
        cv::FONT_HERSHEY_DUPLEX, .75, cv::Scalar(255, 255, 0), 2, 8, false);
  else if (index == 1)
    putText(
        img_rgb, text, cv::Point(20, img_rgb.rows - 70),
        cv::FONT_HERSHEY_DUPLEX, .75, cv::Scalar(255, 0, 0), 2, 8, false);
  else if (index == 2)
    putText(
        img_rgb, text, cv::Point(20, img_rgb.rows - 70),
        cv::FONT_HERSHEY_DUPLEX, .75, cv::Scalar(0, 255, 0), 2, 8, false);

  sprintf(
      text, "# Matches: %d, # Inliers: %d, Ratio %.2f", nmatches, ninliers,
      ratio);

  if (index == 0)
    putText(
        img_rgb, text, cv::Point(20, img_rgb.rows - 30),
        cv::FONT_HERSHEY_DUPLEX, .75, cv::Scalar(255, 255, 0), 2, 8, false);
  else if (index == 1)
    putText(
        img_rgb, text, cv::Point(20, img_rgb.rows - 30),
