...
void KAZE::Allocate_Memory_Evolution() {
  ...
  for (int i = 0; i <= options_.omax - 1; i++) {
    for (int j = 0; j <= options_.nsublevels - 1; j++) {
      ...
      step.esigma =
          options_.soffset *
          pow((float)2.0, (float)(j) / (float)(options_.nsublevels) + i);
...
int KAZE::Create_Nonlinear_Scale_Space(const cv::Mat& img) {
  ...
  if (evolution_.size() == 0) {
    ...
    cout << "Firstly you need to call KAZE::Allocate_Memory_Evolution()"
         << endl;
    return -1;
  }
  ...
  gaussian_2D_convolution(
      evolution_[0].Lt, evolution_[0].Lt, 0, 0, options_.soffset);
  gaussian_2D_convolution(
      evolution_[0].Lt, evolution_[0].Lsmooth, 0, 0, options_.sderivatives);

  // Allocate memory for the flow and step images
  cv::Mat Lflow =
      cv::Mat::zeros(evolution_[0].Lt.rows, evolution_[0].Lt.cols, CV_32F);
  cv::Mat Lstep =
      cv::Mat::zeros(evolution_[0].Lt.rows, evolution_[0].Lt.cols, CV_32F);
  ...
  if (options_.verbosity == true) {
    cout << "Computed image evolution step. Evolution time: "
         << evolution_[0].etime << " Sigma: " << evolution_[0].esigma << endl;
  }
  ...
  for (size_t i = 1; i < evolution_.size(); i++) {
    ...
    gaussian_2D_convolution(
        evolution_[i - 1].Lt, evolution_[i].Lsmooth, 0, 0,
        options_.sderivatives);

    // Compute the Gaussian derivatives Lx and Ly
    cv::Scharr(
        evolution_[i].Lsmooth, evolution_[i].Lx, CV_32F, 1, 0, 1, 0,
        cv::BORDER_DEFAULT);
    cv::Scharr(
        evolution_[i].Lsmooth, evolution_[i].Ly, CV_32F, 0, 1, 1, 0,
        cv::BORDER_DEFAULT);
    ...
    // Perform the evolution step with AOS
      ...
      AOS_Step_Scalar(
          evolution_[i].Lt, evolution_[i - 1].Lt, Lflow,
          evolution_[i].etime - evolution_[i - 1].etime);

    if (options_.verbosity == true) {
      cout << "Computed image evolution step " << i
           << " Evolution time: " << evolution_[i].etime
...
void KAZE::Compute_KContrast(const cv::Mat& img) {
  ...
  options_.kcontrast = compute_k_percentile(
      img, options_.kcontrast_percentile, options_.sderivatives,
      options_.kcontrast_nbins, 0, 0);
...
#pragma omp parallel for
  ...
  for (int i = 0; i < (int)(evolution_.size()); i++) {
    if (options_.verbosity == true) {
      cout << "Computing multiscale derivatives. Evolution time: "
           << evolution_[i].etime
    ...
    compute_scharr_derivatives(
        evolution_[i].Lsmooth, evolution_[i].Lx, 1, 0,
        evolution_[i].sigma_size);
    compute_scharr_derivatives(
        evolution_[i].Lsmooth, evolution_[i].Ly, 0, 1,
        evolution_[i].sigma_size);
    compute_scharr_derivatives(
        evolution_[i].Lx, evolution_[i].Lxx, 1, 0, evolution_[i].sigma_size);
    compute_scharr_derivatives(
        evolution_[i].Ly, evolution_[i].Lyy, 0, 1, evolution_[i].sigma_size);
    compute_scharr_derivatives(
        evolution_[i].Lx, evolution_[i].Lxy, 0, 1, evolution_[i].sigma_size);

    evolution_[i].Lx = evolution_[i].Lx * ((evolution_[i].sigma_size));
    evolution_[i].Ly = evolution_[i].Ly * ((evolution_[i].sigma_size));
    evolution_[i].Lxx = evolution_[i].Lxx * ((evolution_[i].sigma_size) *
                                             (evolution_[i].sigma_size));
    evolution_[i].Lxy = evolution_[i].Lxy * ((evolution_[i].sigma_size) *
                                             (evolution_[i].sigma_size));
    evolution_[i].Lyy = evolution_[i].Lyy * ((evolution_[i].sigma_size) *
                                             (evolution_[i].sigma_size));
  }
...
#pragma omp parallel for
  ...
  for (size_t i = 0; i < kpts_par_.size(); i++) {
    for (size_t j = 0; j < kpts_par_[i].size(); j++) {
      ...
      // Check in case we have the same point as maxima in previous evolution
      // levels
      for (size_t ik = 0; ik < kpts.size(); ik++) {
        if (kpts[ik].class_id == level || kpts[ik].class_id == level + 1 ||
            kpts[ik].class_id == level - 1) {
          dist = (kpts_par_[i][j].pt.x - kpts[ik].pt.x) *
                     (kpts_par_[i][j].pt.x - kpts[ik].pt.x) +
                 (kpts_par_[i][j].pt.y - kpts[ik].pt.y) *
                     (kpts_par_[i][j].pt.y - kpts[ik].pt.y);

          if (dist <
              evolution_[level].sigma_size * evolution_[level].sigma_size) {
    ...
    for (int jx = 1; jx < options_.img_width - 1; jx++) {
      ...
      if (value > options_.dthreshold && value >= 0.00001) {
        if (value >= *(evolution_[level].Ldet.ptr<float>(ix) + jx - 1)) {
          ...
          if (check_maximum_neighbourhood(
                  evolution_[level].Ldet, 1, value, ix, jx, 1)) {
            // Now check on the lower scale
            if (check_maximum_neighbourhood(
                    evolution_[level - 1].Ldet, 1, value, ix, jx, 0)) {
...
void KAZE::Do_Subpixel_Refinement(std::vector<cv::KeyPoint>& kpts) {
  ...
  for (size_t i = 0; i < kpts_.size(); i++) {
    ...
    Dys = (1.0 / (4.0 * step)) *
              (*(evolution_[kpts_[i].class_id + 1].Ldet.ptr<float>(y + step) +
                 x) +
               (*(evolution_[kpts_[i].class_id - 1].Ldet.ptr<float>(y - step) +
                  x))) -
          (1.0 / (4.0 * step)) *
    ...
    if (fabs(dst(0)) <= 1.0 && fabs(dst(1)) <= 1.0 && fabs(dst(2)) <= 1.0) {
      ...
      kpts_[i].pt.y += dst(1);
      dsc = kpts_[i].octave +
            (kpts_[i].angle + dst(2)) / ((float)(options_.nsublevels));
...
void KAZE::Compute_Main_Orientation(cv::KeyPoint& kpt) {
  ...
  for (int i = -6; i <= 6; ++i) {
    for (int j = -6; j <= 6; ++j) {
      if (i * i + j * j < 36) {
        ...
        if (iy >= 0 && iy < options_.img_height && ix >= 0 &&
            ix < options_.img_width) {
  ...
  for (ang1 = 0; ang1 < 2.0 * CV_PI; ang1 += 0.15f) {
    ang2 =
        (ang1 + CV_PI / 3.0f > 2.0 * CV_PI ? ang1 - 5.0f * CV_PI / 3.0f
                                           : ang1 + CV_PI / 3.0f);
    sumX = sumY = 0.f;
...
/* ************************************************************************* */
void KAZE::Get_SURF_Upright_Descriptor_64(
    const cv::KeyPoint& kpt, float* desc) {
  float dx = 0.0, dy = 0.0, mdx = 0.0, mdy = 0.0;
  float rx = 0.0, ry = 0.0, len = 0.0, xf = 0.0, yf = 0.0, sample_x = 0.0,
        sample_y = 0.0;
...
/* ************************************************************************* */
void KAZE::Get_MSURF_Upright_Descriptor_64(
    const cv::KeyPoint& kpt, float* desc) {
...
/* ************************************************************************* */
void KAZE::Get_GSURF_Upright_Descriptor_64(
    const cv::KeyPoint& kpt, float* desc) {
  float dx = 0.0, dy = 0.0, mdx = 0.0, mdy = 0.0;
  float rx = 0.0, ry = 0.0, rxx = 0.0, rxy = 0.0, ryy = 0.0, len = 0.0,
        xf = 0.0, yf = 0.0;
  float sample_x = 0.0, sample_y = 0.0;
  float fx = 0.0, fy = 0.0, res1 = 0.0, res2 = 0.0, res3 = 0.0, res4 = 0.0;
      ...
      for (int k = i; k < i + sample_step; k++) {
        for (int l = j; l < j + sample_step; l++) {
          ...
          if (modg != 0.0) {
            ...
            rxx = (1.0 - fx) * (1.0 - fy) * res1 + fx * (1.0 - fy) * res2 +
                  (1.0 - fx) * fy * res3 + fx * fy * res4;
            ...
            rxy = (1.0 - fx) * (1.0 - fy) * res1 + fx * (1.0 - fy) * res2 +
                  (1.0 - fx) * fy * res3 + fx * fy * res4;
            ...
            ryy = (1.0 - fx) * (1.0 - fy) * res1 + fx * (1.0 - fy) * res2 +
                  (1.0 - fx) * fy * res3 + fx * fy * res4;

            // Lww = (Lx^2 * Lxx + 2*Lx*Lxy*Ly + Ly^2*Lyy) / (Lx^2 + Ly^2)
            lww = (pow(rx, 2) * rxx + 2.0 * rx * rxy * ry + pow(ry, 2) * ryy) /
                  (modg);

            // Lvv = (-2*Lx*Lxy*Ly + Lxx*Ly^2 + Lx^2*Lyy) / (Lx^2 + Ly^2)
            lvv = (-2.0 * rx * rxy * ry + rxx * pow(ry, 2) + pow(rx, 2) * ryy) /
                  (modg);
          } else {
            lww = 0.0;
            lvv = 0.0;
...
/* ************************************************************************* */
void KAZE::Get_SURF_Upright_Descriptor_128(
    const cv::KeyPoint& kpt, float* desc) {
  float rx = 0.0, ry = 0.0, len = 0.0, xf = 0.0, yf = 0.0, sample_x = 0.0,
        sample_y = 0.0;
  float fx = 0.0, fy = 0.0, res1 = 0.0, res2 = 0.0, res3 = 0.0, res4 = 0.0;
  ...
  for (int i = -pattern_size; i < pattern_size; i += sample_step) {
    for (int j = -pattern_size; j < pattern_size; j += sample_step) {
      ...
      // Store the current length^2 of the vector
      len += dxp * dxp + dxn * dxn + mdxp * mdxp + mdxn * mdxn + dyp * dyp +
             dyn * dyn + mdyp * mdyp + mdyn * mdyn;
    }
  }
...
/* ************************************************************************* */
void KAZE::Get_MSURF_Upright_Descriptor_128(
    const cv::KeyPoint& kpt, float* desc) {
...
/* ************************************************************************* */
void KAZE::Get_GSURF_Upright_Descriptor_128(
    const cv::KeyPoint& kpt, float* desc) {
...
/* ************************************************************************* */
void KAZE::AOS_Step_Scalar(
    cv::Mat& Ld, const cv::Mat& Ldprev, const cv::Mat& c,
...
/* ************************************************************************* */
void KAZE::AOS_Rows(
    const cv::Mat& Ldprev, const cv::Mat& c, const float stepsize) {
  // Operate on rows
  for (int i = 0; i < qr_.rows; i++) {
    for (int j = 0; j < qr_.cols; j++) {
      *(qr_.ptr<float>(i) + j) =
          *(c.ptr<float>(i) + j) + *(c.ptr<float>(i + 1) + j);
    }
  }
  ...
  for (int i = 1; i < py_.rows - 1; i++) {
    for (int j = 0; j < py_.cols; j++) {
      *(py_.ptr<float>(i) + j) =
          *(qr_.ptr<float>(i - 1) + j) + *(qr_.ptr<float>(i) + j);
    }
  }
...
/* ************************************************************************* */
void KAZE::AOS_Columns(
    const cv::Mat& Ldprev, const cv::Mat& c, const float stepsize) {
  // Operate on columns
  for (int j = 0; j < qc_.cols; j++) {
    for (int i = 0; i < qc_.rows; i++) {
      *(qc_.ptr<float>(i) + j) =
          *(c.ptr<float>(i) + j) + *(c.ptr<float>(i) + j + 1);
    }
  }
  ...
  for (int j = 1; j < px_.cols - 1; j++) {
    for (int i = 0; i < px_.rows; i++) {
      *(px_.ptr<float>(i) + j) =
          *(qc_.ptr<float>(i) + j - 1) + *(qc_.ptr<float>(i) + j);
    }
  }
...
/* ************************************************************************* */
void KAZE::Thomas(
    const cv::Mat& a, const cv::Mat& b, const cv::Mat& Ld, cv::Mat& x) {
  // Auxiliary variables
  int n = a.rows;
  ...
  for (int k = 1; k < n; k++) {
    for (int j = 0; j < l.cols; j++) {
      *(l.ptr<float>(k - 1) + j) =
          *(b.ptr<float>(k - 1) + j) / *(m.ptr<float>(k - 1) + j);
    }

    for (int j = 0; j < m.cols; j++) {
      *(m.ptr<float>(k) + j) =
          *(a.ptr<float>(k) + j) -
          *(l.ptr<float>(k - 1) + j) * (*(b.ptr<float>(k - 1) + j));
    }

    for (int j = 0; j < y.cols; j++) {
      *(y.ptr<float>(k) + j) =
          *(Ld.ptr<float>(k) + j) -
          *(l.ptr<float>(k - 1) + j) * (*(y.ptr<float>(k - 1) + j));
    }
  }
  ...
  for (int j = 0; j < y.cols; j++) {
    *(x.ptr<float>(n - 1) + j) =
        (*(y.ptr<float>(n - 1) + j)) / (*(m.ptr<float>(n - 1) + j));
  }
  ...
  for (int i = n - 2; i >= 0; i--) {
    for (int j = 0; j < x.cols; j++) {
      *(x.ptr<float>(i) + j) =
          (*(y.ptr<float>(i) + j) -
           (*(b.ptr<float>(i) + j)) * (*(x.ptr<float>(i + 1) + j))) /
          (*(m.ptr<float>(i) + j));
    }
  }
}
...
/* ************************************************************************* */
inline void libKAZE::checkDescriptorLimits(
