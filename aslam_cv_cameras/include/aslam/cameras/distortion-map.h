...
class DistortionMap {
 public:
  ...
  void set(const unsigned& col, const unsigned& row, const Eigen::Vector2d& point) {
    distortion_map_[row * image_width_ + col] = point;
  }
  Eigen::Vector2d& get(const unsigned& col, const unsigned& row, Eigen::Vector2d* point) {
