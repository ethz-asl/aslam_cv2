...
bool VisualNPipeline::processImageBlockingIfFull(
    ...
    size_t max_queue_size, const std::string& encoding) {
  ...
  while (!shutdown_) {
    ...
    processImageImpl(camera_index, image, timestamp, encoding);
    return true;
  }
  return false;
}
...
bool VisualNPipeline::processImageNonBlockingDroppingOldestNFrameIfFull(
    size_t camera_index, const cv::Mat &image, int64_t timestamp,
    size_t max_output_queue_size, const std::string& encoding) {
...
void VisualNPipeline::processImage(
    size_t camera_index, const cv::Mat& image, int64_t timestamp, const std::string& encoding) {
...
void VisualNPipeline::work(size_t camera_index, const cv::Mat& image,
                           int64_t timestamp_nanoseconds, const std::string& encoding) {
  CHECK_LE(camera_index, pipelines_.size());
  std::shared_ptr<VisualFrame> frame;
  frame = pipelines_[camera_index]->processImage(image, timestamp_nanoseconds, encoding);
