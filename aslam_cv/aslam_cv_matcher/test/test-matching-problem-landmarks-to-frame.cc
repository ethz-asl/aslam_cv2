...
TEST_F(LandmarksToFrameMatcherTest, MatchIdentity) {
  ...
  const Eigen::Matrix<unsigned char, 48, 1> frame_descriptors =
      Eigen::Matrix<unsigned char, 48, 1>::Zero();
  const Eigen::Matrix<unsigned char, 48, 1> landmark_descriptors =
      Eigen::Matrix<unsigned char, 48, 1>::Zero();
...
TEST_F(LandmarksToFrameMatcherTest, MatchRandomly) {
  ...
  for (size_t keypoint_index = 0u; keypoint_index < kNumKeypoints; ++keypoint_index) {
    const double keypoint_x = static_cast<double>(std::rand() % camera_->imageWidth());
    const double keypoint_y = static_cast<double>(std::rand() % camera_->imageHeight());

    frame_keypoints(0, keypoint_index) = keypoint_x;
    frame_keypoints(1, keypoint_index) = keypoint_y;

    const double shift_translation_pixels = static_cast<double>(image_space_distance_threshold_ -
                                      (std::rand() % 2 * image_space_distance_threshold_));
    const double shift_angle_radians = static_cast<double>(std::rand() % 360) / 180.0 * 3.14159;

    const double x_shift = std::cos(shift_translation_pixels);
    const double y_shift = std::sin(shift_translation_pixels);
  ...
  const Eigen::Matrix<unsigned char, 48, Eigen::Dynamic> frame_descriptors =
      Eigen::Matrix<unsigned char, 48, Eigen::Dynamic>::Random(48, kNumKeypoints);

  const Eigen::Matrix<unsigned char, 48, Eigen::Dynamic> landmark_descriptors =
      frame_descriptors;
