...
bool MatchingProblemLandmarksToFrameMultimap::doSetup() {
  aslam::timing::Timer method_timer("MatchingProblemLandmarksToFrameMultimap::doSetup()");
  ...
  for (size_t keypoint_idx = 0u; keypoint_idx < num_keypoints; ++keypoint_idx) {
    ...
    if (camera->isMasked(keypoint)) {
      ...
      CHECK_LT(y_coordinate, image_height_) << "The y coordinate for keypoint "
...
void MatchingProblemLandmarksToFrameMultimap::getAppleCandidatesForBanana(
  ...
  CHECK_GT(image_height_, 0u) << "The image height of the visual frame is zero.";
  ...
  if (is_landmark_valid_[landmark_index]) {
    ...
    CHECK_LT(y_lower, image_height_) << "The y coordinate of the lower search band bound "
    ...
    for (std::multimap<size_t, size_t>::iterator it = it_lower; it != it_upper; ++it) {
      ...
      if (squared_image_space_distance < squared_image_space_distance_threshold_px_sq_) {
        ...
        if (hamming_distance < hamming_distance_threshold_) {
          ...
          const int kCandidatePriority = 0;
          candidates->emplace_back(
              frame_keypoint_index, landmark_index, computeMatchScore(hamming_distance),
              kCandidatePriority);
        } else {
