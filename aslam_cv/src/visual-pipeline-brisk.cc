...
BriskVisualPipeline::BriskVisualPipeline(
  ...
  input_camera_ = camera;
  output_camera_ = camera;
  preprocessing_.reset(new NullVisualPipeline(camera, copy_images));
...
BriskVisualPipeline::BriskVisualPipeline(
    const VisualPipeline::Ptr& preprocessing, size_t octaves,
    double uniformity_radius, double absolute_threshold,
    size_t max_number_of_keypoints, bool rotation_invariant,
    bool scale_invariant) : preprocessing_(preprocessing) {
  CHECK_NOTNULL(preprocessing.get());
  input_camera_ = preprocessing->getInputCamera();
  output_camera_ = preprocessing_->getOutputCamera();
  initializeBrisk(octaves, uniformity_radius, absolute_threshold,
                  max_number_of_keypoints, rotation_invariant,
                  scale_invariant);
}
...
void BriskVisualPipeline::processFrame(
    const cv::Mat& input_image, std::shared_ptr<VisualFrame>* frame) const {
  // Undistort or do whatever you do here.
  preprocessing_->processFrame(input_image, frame);
