...
#include <aslam/descriptors/hamming.h>
...
inline void FlipNRandomBits(size_t num_bits_to_flip, FeatureDescriptorRef* descriptor) {
  ...
  for (size_t i = 0; i < num_bits_to_flip; ++i) {
    common::FlipBit(bits_to_flip[i], descriptor);
  }
}
...
inline void Serialize(const FeatureDescriptorRef& value, std::ostream* out);

inline void Deserialize(FeatureDescriptorRef* value, std::istream* in);
}  // namespace common
