...
#include <aslam/common/binary-serialization.h>
...
void Serialize(const FeatureDescriptorRef& value, std::ostream* out) {
  uint32_t size = value.size();
  common::Serialize(size, out);
  common::Serialize(value.data(), value.size(), out);
}

void Deserialize(FeatureDescriptorRef* value, std::istream* in) {
  CHECK_NOTNULL(value);
  uint32_t size;
