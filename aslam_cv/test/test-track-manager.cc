...
TEST(TrackManagerTests, TestApplyMatcher) {
  ...
  matches.emplace_back(1, 1, 0.2);
  matches.emplace_back(2, 2, 0.3);
  matches.emplace_back(3, 3, 0.4);
  ...
  expected_banana_tracks << 3, 2, 0, 1, 4;

  Eigen::VectorXi expected_apple_tracks(5);
  expected_apple_tracks << 3, 2, 0, 1, 4;
...
TEST(TrackManagerTests, TestApplyMatchesUniformly) {
  ...
  for (size_t match_idx = 0; match_idx < kNumKeypoints; ++match_idx) {
    ...
    matches.emplace_back(match_idx, match_idx, score);
  }
